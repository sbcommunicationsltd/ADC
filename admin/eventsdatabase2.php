...
if(isset($_POST['submitedit']))
{
	...
	{
		if($field!='Address_Town' && $field!='Address_City' && $field!='Image_Path' && $field!='City')
		{
			$formvar = $_POST[$field];
			if ($formvar=="")
			{
...
if(isset($_GET['edit']))
{
	...
	<table width='600' align='center' style="border:1px solid #d0d3d5; background-color:transparent;" cellspacing='2' cellpadding='2'>
		<tr>
			<th colspan="2" bgcolor="#ec008c"><div style="color: #00225d;">Edit this Event</div></td>
		</tr>
		...
		<tr>
			...
			<?php $types = array('Networking Drinks', 'Dinner Party', 'Valentines Dinner', 'Christmas Lunch', 'Sunday Brunch', 'Cheese and Wine', 'Fine Art/Fine Dining', 'Champagne and Canapes', 'Summer Party', 'Sunday Lunch', 'Party', 'Launch Party', 'Thanksgiving Dinner', 'Diwali Party');
			foreach($types as $ty)
			{
		...
		<tr>
			...
			<td valign='middle'><select name="Religion">
			<?php $rels = array('Hindu/Sikh', ' Muslim', 'All', 'Sikh only');
			foreach($rels as $rel)
			{
				echo "<option value='$rel' "; if(isset($_POST['Religion'])){if($_POST['Religion'] == $rel){echo "selected='selected'";}}else{if($row2['Religion']== $rel) { echo "selected='selected'";}} echo ">$rel</option>";
			}?></select></td>
		</tr>
		<tr>
			<th align='left'>Type of Membership</th>
			<td valign='middle'><select name='Member_Type'><option value=''>Select</option>
				<option value='Standard' <?php if(isset($_POST['Member_Type'])){if($_POST['Member_Type'] == 'Standard'){echo "selected='selected'";}}else{if($row2['Member_Type'] == 'Standard'){echo "selected='selected'";}}?>>Standard</option>
				<option value='Premier' <?php if(isset($_POST['Member_Type'])){if($_POST['Member_Type'] == 'Premier'){echo "selected='selected'";}}else{if($row2['Member_Type'] == 'Premier'){echo "selected='selected'";}}?>>Premier</option>
			</select></td>
		</tr>
		<!--<tr>
			<th align='left'>City</th>
			<td valign='middle'><select name="City">
			<?php $cities = array('London', 'Paris', 'New York', 'Dubai', 'Leeds', 'Manchester', 'Birmingham');
		...
		<tr>
			...
			<td><select name="Time"><option value="">Time</option>
				<?php
				$hours = array('11.30', '11.45');
				foreach($hours as $hour)
				{
					echo "<option value='$hour'"; if(isset($_POST['Time'])){if($_POST['Time'] == $time){echo "selected='selected'";}}else{if($row2['Time']==$hour) {echo "selected='selected'";}} echo ">$hour am</option>";
				}
				for ($i = 0; $i <= 9; $i++)
				{
    				if($i == 9)
...
if(isset($_POST['submitadd']))
{
	...
	foreach ($fieldname as $field)
	{
		if($field!='Address_Town' && $field!='Address_City' && $field!='Image_Path' && $field!='ID' && $field!='City')
		{
			$formvar = $_POST[$field];
			if ($formvar=="")
			{
...
{?>
	<form method='post' name='addevent' enctype="multipart/form-data">
		<table width='600' align='center' style="border:1px solid #d0d3d5; background-color:transparent;" cellspacing='2' cellpadding='2'>
			<tr>
				<th colspan="2" bgcolor="#ec008c"><div style="color: #00225d;">Add an Event</div></td>
			</tr>
			<tr>
				<th align='left'>Date</th>
				<td><select name="Date_Day"><option value="">Day</option>
			...
			<th align='left'>Religion</th>
				...
				foreach($rels as $rel)
				{
					echo "<option value='$rel' "; if($_POST['Religion'] == $rel){echo "selected='selected'";} echo ">$rel</option>";
				}?></select></td>
			</tr>
			<tr>
				<th align='left'>Type of Membership</th>
				<td valign='middle'>
				<select name='Member_Type'><option value=''>Select</option>
				<option value='Standard' <?php if($_POST['Member_Type'] == 'Standard'){echo "selected='selected'";}?>>Standard</option>
				<option value='Premier' <?php if($_POST['Member_Type'] == 'Premier'){echo "selected='selected'";}?>>Premier</option>
				</select></td>
			</tr>
			<!--<tr>
				<th align='left'>City</th>
			...
			<tr>
				...
				<td><select name="Time"><option value="">Time</option>
					...
					foreach($hours as $hour)
					{
						echo "<option value='$hour' "; if($hour == $_POST['Time']){echo "selected='selected'";} echo ">$hour am</option>";
					}
					for ($i = 0; $i <= 9; $i++)
					{
    					if($i == 9)
			...
			<tr>
				<th align='left'>Price</th>
				<td><select name="Price"><option value="">Price</option><option value='0' <?php if($_POST['Price'] == 0){echo "selected='selected'";}?>>Free</option>
					<?php
					$pr = 10;
					while($pr<=100)
					{
...
if((!isset($_GET['edit'])) && (!isset($_POST['submitedit'])) && (!isset($_GET['add'])) && (!isset($_POST['submitadd'])))
{
	...
	if($num_rows != 0)
	{
		while($row = mysql_fetch_array($result2))
		{
			...
			<table width='600' align='center' style="border:1px solid #d0d3d5; background-color:transparent;" cellspacing='2' cellpadding='2'>
				<tr>
					<th colspan="2" bgcolor="#ec008c"><div style="color: #00225d;">Event <?php echo $page;?></div></td>
				</tr>
				<tr>
